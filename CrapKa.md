...
| B12 | ✅ fait | **[Nomistek v1.3.0] Tie-break `triggerIdx` dans `_bruteForce`** — Quand plusieurs séquences terminent avec le même score ET la même longueur, le BF sélectionnait la première trouvée (ordre d'exploration), pouvant placer un coup non-essentiel avant la crapette. Fix : troisième critère dans la boucle de sélection — si `triggerIdx` (index du premier coup crapette/découverte dans `seq.moves[]`) est plus faible, cette séquence est préférée. Condition : `seq.triggerIdx>=0 && (best.triggerIdx<0 \|\| seq.triggerIdx<best.triggerIdx)`. Ex : 612.0#T→D*(cr) (triggerIdx=1) éliminé en faveur de 612.0#D*(cr)→T (triggerIdx=0). |
