...
| B9 | ✅ fait | **[v1.2.31] IA joue des coups inutiles avant de vider la pile par D♦** — Après V→P1 (tNum=11), D♦→P1 (tNum=12→clear forcé→A♦→2♠=crapette) était traité comme un simple coup de main (+5) car `activatesCrapette` ne voyait qu'1 niveau. `_nonKingActivates` aussi. Résultat : BF préférait jouer 5♣, 3♠, 6♦ (+5 chacun) avant D♦, score 545 > 535. Fix : 2 extensions en 2 étapes — (1) `_nonKingActivates` cherche maintenant coup→tNum=12→clear→As→crT playable, (2) `activates2Step` par coup idem, inclus dans `activatesCrapette`. D♦ reçoit +25 au lieu de +5 ; 5♣/3♠/6♦ reçoivent `preCrapettePenalty=−10`. Chemin direct : 555 > 540. |
