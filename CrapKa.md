...
| B8 | ✅ fait | **As de main ignoré pour init pile vide** — `_sLegal` cherchait le premier As dans `_sSources` (ordre : crapette, défausse, main), donc un As de défausse était toujours préféré à un As de main. Première tentative (laisser le BF choisir parmi toutes sources) échouait car le BF trouvait des séquences équivalentes (init_def + défausser_as_main = même score qu'init_main). Fix final : règle stratégique absolue dans `_sLegal` — As de MAIN prioritaire sur As de défausse (un As en main ne sert qu'à init une pile vide, autant l'utiliser immédiatement et conserver l'As de défausse pour les tours futurs). Déduplication renforcée dans `_bfDedup` : as de main/défausse sur piles vides multiples = identiques (`E` key) ; init pioche sur piles vides multiples = identique. |
