...
| B8 | ✅ fait | **Carte de défausse jouée à la place de la même valeur en main** — Règle générale : même valeur en main ET en défausse → prioriser main, SAUF si la carte cachée sous la défausse est jouable sur une pile commune (la révéler vaut le coup). Deux endroits corrigés :<br>• `_findAce()` (coups forcés avant BF) : cherche main avant défausse, exception si carte cachée utile.<br>• `_bfDedup()` (BF) : si même valeur en main et en défausse, et que la défausse ne cache rien d'utile → éliminer le coup défausse. Nouvelle helper `_defHidesUseful(g,ui,pidx,defIdx)`. |
