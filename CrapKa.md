...
| B8 | ✅ fait | **As de main ignoré pour init pile vide** — `_sLegal` cherchait le premier As dans `_sSources` (ordre : crapette, défausse, main), donc un As de défausse était toujours préféré à un As de main. Fix : `_sLegal` génère maintenant un coup par As disponible (toutes sources) ; le BF compare et choisit. Déduplication renforcée dans `_bfDedup` : as de main/défausse sur piles vides multiples = identiques (`E` key) ; init pioche sur piles vides multiples = identique. |
