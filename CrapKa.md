...
| B8 | ✅ fait | **As de main ignoré pour init pile vide** — Le bug était dans `_findAce()` (appelé par `_applyForcedMoves` avant le BF) : ordre crapette → défausse → main ; l'As de défausse était toujours pris avant celui de la main. Fix : `_findAce` vérifie la main AVANT la défausse. Exception : si la carte cachée sous l'As de défausse est jouable sur une pile commune (`_handCardIsPlayable`), on préfère la défausse pour révéler cette carte utile. Règle générale : même valeur en main et en défausse → prioriser main, SAUF si la défausse cache une carte nécessaire pour la crapette ou vider la main. |
