...
| B13 | ✅ fait | **[ai v1.3.1] As posé avant crapette après recyclage** — Après un `clear` (pile commune → futurePioche quand Dame=12 visible), la crapette est directement jouable sur une pile non-vide ET un As de défausse peut remplir la pile vide. Le BF choisissait l'As en premier car son `hMax` (675, crapette non jouée) compensait son faible score immédiat, tandis que la branche crapette-first (hMax=175 après crapette jouée) se retrouvait élagée dès qu'une séquence As-first terminait. Fix (`_sLegal` dans `ai.js`) : si `crapetteBlocksAce = crTop && pile-non-vide reçoit crTop` → ne pas générer les coups As→pile-vide. La crapette est jouée d'abord, puis la pile vide reçoit l'As lors de l'expansion suivante (où crapettePlayed=true → crTop changé → `crapetteBlocksAce=false`). Règle : l'As n'est pas obligatoire immédiatement après recyclage ; si un as apparaît sur la crapette, c'est lui qu'on pose. |
